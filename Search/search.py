    ...
    while not open.isEmpty():
        ...
        for node in problem.getSuccessors(currentNode[0]):
            if node[0] not in close:
                open.push((node[0], currentNode[1] + [node[1]]))
...
def uniformCostSearch(problem):
    ...
    while not open.isEmpty():
        currentNode, path, currentCost = open.pop()

        if problem.isGoalState(currentNode):
            return path

        if currentNode in close and close[currentNode] <= currentCost:
            continue

        close[currentNode] = currentCost

        for node in problem.getSuccessors(currentNode):
            newCost = currentCost + node[2]
            newPath = path + [node[1]]

            open.push((node[0], newPath, newCost,), newCost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not open.isEmpty():
        ...
        for node in problem.getSuccessors(currentNode):
            newCost = currentCost + node[2]
            newPath = path + [node[1]]
            priority = newCost + heuristic(node[0], problem)

            open.push((node[0], newPath, newCost), priority)
